...
INSERT INTO `alumno` VALUES ('1400025G87',NULL,'SSSSSSSSSSSSSSSSSSSSSS','','ELECTRICIDAD','FISICO-MATEMATICO','Quinto','E','MATUTINO','30DCT0236O','jorge@cbtis66.edu.mx','CODJ721205HVZRMR05','lisaSimsomp','','',NULL),('21330050660276','2006-09-17','Landero y coss','2741154955','Programación','Físico Matemático','QUINTO','G','Matutino','30DCT0236O','almn.alexaacevedo@cbtis66.edu.mx','aepa060917mvzcvla9','','','','1'),('2133005660294','2006-06-21','Adolfo Lopez Mateos #310 ','2747416548','Programación','Físico Matemático','QUINTO','G','Matutino','30DCT0236O','almn.ingridduarte@cbtis66.edu.mx','DUGI060621MVRLNA1','Ingrid Duarte Gil','','Celeste Duarte Gil','1'),('22330050660315','2007-07-10','16 de Septiembre 1605','2741156348','Programación','Físico Matemático','TERCER','G','Matutino','30DCT0236O','almn.silviacortes@cbtis66.edu.mx','COTS070710MVZRRLA5','','','','1');
...
INSERT INTO `globales` VALUES (2,'1400025G87','721205','9','1','05/11/2023',0,0,NULL);
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`sigedoes`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `pdf` AS select `alumno`.`numControl` AS `numControl`,`usuario`.`nombre` AS `nombre`,`usuario`.`apellidoP` AS `apellidoP`,`usuario`.`apellidoM` AS `apellidoM`,`alumno`.`direccion` AS `direccion`,`alumno`.`especialidad` AS `especialidad`,`alumno`.`area` AS `area`,`alumno`.`grado` AS `grado`,`alumno`.`grupo` AS `grupo`,`alumno`.`turno` AS `turno`,`alumno`.`horario` AS `horario`,`alumno`.`CTO` AS `CTO`,`especialidad`.`clave` AS `clave`,`escuela`.`Esc_nombre` AS `Esc_nombre`,`escuela`.`Esc_direccion` AS `Esc_direccion`,`escuela`.`Esc_correo` AS `Esc_correo`,`escuela`.`Esc_Director` AS `Esc_Director`,`escuela`.`Esc_telefono` AS `Esc_telefono`,`escuela`.`Esc_Periodo` AS `Esc_Periodo`,`alumno`.`CURP` AS `CURP`,`usuario`.`correo` AS `correo`,`usuario`.`foto` AS `foto` from (((`alumno` join `escuela` on((`escuela`.`CTO` = `alumno`.`CTO`))) join `usuario` on((`usuario`.`numControl` = `alumno`.`numControl`))) join `especialidad` on((`especialidad`.`CTO` = `escuela`.`CTO`))) group by `alumno`.`numControl` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2023-11-13 23:28:16
