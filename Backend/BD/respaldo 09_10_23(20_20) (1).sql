...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `login`(IN `dato1` varchar(14), IN `dato2` varchar(14))
BEGIN
select alumno.nombre as nombre, usuario.rol as rol, usuario.numControl as numControl 
from usuario join alumno on usuario.numControl = alumno.numControl
where usuario.numControl like dato1 and usuario.password = dato2 and alumno.alta = 1;
END ;;
DELIMITER ;
...
-- Dump completed on 2023-10-09 20:22:45
