...
function MailOptions(tipo, email) {
  ...
  switch (tipo) {
    ...
    case 'forgotPassword':
      ...
      return mailOptions2;
    case 'validacion':
      const validacion = (email.op == 1)? `Aceptado sus datos son: </h3> <br><h5>Nombre: ${email.nombre}   Usuario: ${email.numControl}   contraseña: ${email.password}</h5>`: "Rechazado </h3>, <br><h5>favor de enviar un correo a a la siguiente direccion: Direcion@cbtis66.edu.mx</h5>"; 
      const mailOptions3 = {
        from: `"Control escolar", "jorgecortescbtis66@gmail.com"`,
        to: `"${email.correo}"`,
